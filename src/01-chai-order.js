...
export function hasSpecialIngredient(order, ingredient) {
  // Your code here
  if (typeof order !== "string" || order.trim() === "" || typeof ingredient !== "string" || ingredient.trim() === "") return false

  if (order.trim().toLowerCase().includes(ingredient.trim().toLowerCase())) return true
  return false
}

...
export function repeatPattern(pattern, times) {
  // Your code here
  if (typeof pattern !== "string" || pattern.trim() === "" || !Number.isInteger(times) || times <= 0) return ""
...
export function extractRangoliCenter(design, start, end) {
  // Your code here
  if (typeof design !== "string" || design === "" || !Number.isInteger(start) || !Number.isInteger(end)) return "";
  return design.slice(start, end);
}

export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  // Your code here
  if (typeof colorString !== "string" || colorString === "") return "";
...
export function makeRangoliBorder(char, length) {
  // Your code here
  if (typeof char !== "string" || char === "" || !Number.isInteger(length) || length <= 0) return ""
